...
func (instance *okxInstance) ReadMessages() {
	defer logs.Logger.Info("读取信息关闭")
	defer instance.wg.Done() // 在函数退出时递减计数器
	for {
		_, message, err := instance.conn.ReadMessage()
		if err != nil {
			logs.Logger.Error("读取信息失败:" + err.Error())
			go instance.heartbeatMessage()
			instance.wg.Add(1)
			break
		}
		logs.Logger.Info("读取到的消息:", zap.String("tickers：", string(message)))
...
func (instance *okxInstance) handleMessages() {
	defer logs.Logger.Info("处理信息关闭")
	defer instance.wg.Done() // 在函数退出时递减计数器
	for {
		message := <-instance.data
		logs.Logger.Info("tickers：" + string(message))
		data := &Message{}
...
func (instance *okxInstance) heartbeatMessage() {
	defer logs.Logger.Info("关闭okx心跳")
	for {
		// 发送心跳消息
		...
		if err != nil {
			...
			for i := 0; i < instance.maxReconnect; i++ {
				...
				logs.Logger.Info("重新连接:" + strconv.Itoa(instance.maxReconnect))
