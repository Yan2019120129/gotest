...
func (_OkexStruct *OkexStruct) Reader() {
	...
	for {
		...
		if err != nil {
			logger.Logger.Debug("okex ====> 连接已关闭~  5s后进行重新连接....", zap.String("data", err.Error()))
...
func (_OkexStruct *OkexStruct) Subscribe(msg *Subscribe) {
	if _OkexStruct.conn != nil {
		logger.Logger.Info("info", zap.Reflect("message", msg))
...
func (_OkexStruct *OkexStruct) TickerUpdatePrice() {
	...
	for symbol, ticker := range _OkexStruct.CurrentTickers {
		...
		logger.Logger.Info("info", zap.Reflect("update", symbol))
		logger.Logger.Info("info", zap.Reflect("TickerData", tickerBytes))
